...
<div class="max-w-3xl">
    ...
    <div class="bg-white rounded-xl border border-gray-100 shadow-sm">
        <div class="p-6 border-b border-gray-100">
            ...
            <p class="text-sm text-gray-500 mt-1">Create a new car listing</p>
        </div>

        <form action="{{ route('admin.cars.store') }}" method="POST" enctype="multipart/form-data" class="p-6 space-y-6">
            @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input 
                        ...
                        placeholder="e.g., ₹12.5 - 15.8 Lakh"
                        required
                    >
                    @error('price')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                ...
                <div>
                    ...
                    >
                        @foreach ($listingTypes as $key => $label)
                            <option value="{{ $key }}" {{ old('listing_type') === $key ? 'selected' : '' }}>{{ $label }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input 
                        ...
                        value="{{ old('tag') }}"
                        class="w-full px-4 py-2.5 rounded-lg border border-gray-300 focus:border-primary focus:ring-2 focus:ring-primary/20 outline-none"
                        placeholder="e.g., NEW, POPULAR, TOP SELLER"
                    >
                </div>
                <div>
                ...
                <div class="border-2 border-dashed border-gray-300 rounded-lg p-6 text-center hover:border-primary transition-colors">
                    ...
                    <label for="image" class="cursor-pointer">
                        <div id="imagePreview" class="hidden mb-3">
                            <img src="" alt="Preview" class="w-full max-w-md mx-auto h-48 object-contain rounded">
                        </div>
                        <div id="imageUploadText">
                            ...
                            <p class="text-sm text-gray-500">Click to upload car image</p>
            ...
            <div class="flex flex-wrap gap-6">
                ...
                <label class="flex items-center gap-3 cursor-pointer">
                    ...
                    <input type="checkbox" name="is_upcoming" value="1" {{ old('is_upcoming') ? 'checked' : '' }} class="w-5 h-5 rounded border-gray-300 text-primary focus:ring-primary">
                    <span class="text-sm font-medium text-gray-700">Upcoming</span>
                </label>
                <label class="flex items-center gap-3 cursor-pointer">
                    <input type="hidden" name="is_active" value="0">
                    <input type="checkbox" name="is_active" value="1" {{ old('is_active', true) ? 'checked' : '' }} class="w-5 h-5 rounded border-gray-300 text-primary focus:ring-primary">
